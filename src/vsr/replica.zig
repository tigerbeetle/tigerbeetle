...
const ReceiveBuffer = @import("../receive_buffer.zig").ReceiveBuffer;
...
pub fn ReplicaType(
    ...
    return struct {
        ...
        fn on_messages_from_bus(message_bus: *MessageBus, buffer: *ReceiveBuffer) void {
            const self: *Replica = @alignCast(@fieldParentPtr("message_bus", message_bus));
            self.on_messages(buffer);
        }

        pub fn on_messages(self: *Replica, buffer: *ReceiveBuffer) void {
