...
const Errors = struct {
    count: u32 = 0,
    captured: ?std.ArrayListUnmanaged(u8) = null,
    ...
    pub fn add_bad_type_function_name(
        ...
        const line_number = line_index + 1;
        errors.emit(
            "{s}:{d}: error: type functon name '{s}' should end in 'Type'\n",
    ...
    pub fn add_invalid_markdown_title(errors: *Errors, file: SourceFile) void {
        errors.emit(
            "{s}: error: document should have exatly one top-level '# Title'\n",
            .{file.path},
        );
    }
...
fn tidy_banned(file: SourceFile, errors: *Errors) void {
    // Vendored code is excempt from bans.
...
test tidy_type_functions {
    ...
    ,
        snap(@src(),
            \\type_functions.zig:2: error: type functon name 'MyArray' should end in 'Type'
            \\
        ),
    );
}
...
test tidy_markdown_title {
    ...
    ,
        snap(@src(),
            \\bad.md: error: document should have exatly one top-level '# Title'
            \\
        ),
    );
}
