...
pub fn GridType(comptime Storage: type) type {
    ...
    return struct {
        ...
        /// Entries in `blocks` correspond to entries in `blocks_references`.
        ...
        // Invariants:
        // - `stash_free.len + stash_used.len == stash_blocks_count`
        ...
        pub fn init(allocator: mem.Allocator, options: struct {
            ...
            try stash_free.ensureTotalCapacity(allocator, stash_blocks_count);
            errdefer stash_free.deinit(allocator);

            var stash_used = std.AutoArrayHashMapUnmanaged(u32, void).empty;
            try stash_used.ensureTotalCapacity(allocator, stash_blocks_count);
            errdefer stash_used.deinit(allocator);
        ...
        pub fn get_block(grid: *Grid) BlockPtr {
            ...
            const stash_location = stash_entry.key;
        ...
        pub fn fulfill_block(grid: *Grid, block: BlockPtrConst) bool {
            ...
            while (reads_iterator.next()) |read| {
                if (read.checksum == block_header.header_tag and
        ...
        fn write_block(
            ...
            grid.assert_coherent(header.address, header.header_tag);

            const block_location = grid.location_from_block(block.*);
            assert(grid.stash_used.contains(block_location));
            assert(grid.blocks_references[block_location] > 0);
            if (grid.blocks_references[block_location] > 1) {
        ...
        fn write_block_callback(completion: *Storage.Write) void {
            ...
            grid.assert_coherent(completed_write.address, cache_block_header.header_tag);
        ...
        fn read_block_from_write_queues(
            ...
            while (write_queue_iterator.next()) |queued_write| {
                ...
                if (address == queued_write_header.address and
                    checksum == queued_write_header.header_tag)
                {
                    block_found_count += 1;
        ...
        fn read_block_callback(completion: *Storage.Read) void {
            ...
            const block = grid.read_iop_blocks[iop_index];
            const block_location = grid.location_from_block(block);
            ...
            // This is the block reference "burst", since we hold the current read block while still
            // acquiring a new read block for the iop.
            grid.read_iop_blocks[iop_index] = grid.get_block();
            ...
            if (result == .valid) {
                if (read.cache_write) {
                    grid.cache_upsert(read.address, block);
                }
            } else {
        ...
        fn read_block_validate(block: BlockPtrConst, expect: struct {
            ...
            if (header.header_tag != expect.checksum) return .unexpected_checksum;
        ...
        fn read_block_resolve(grid: *Grid, read: *Grid.Read, result: ReadBlockResult) void {
            ...
            if (result == .valid) {
                ...
                assert(header.header_tag == read.checksum);
            }
        ...
        pub fn verify_table(grid: *Grid, index_address: u64, index_checksum: u128) void {
            ...
            assert(index_block_header.header_tag == index_checksum);
            ...
            for (
                ...
                assert(value_block_header.header_tag == value_checksum.value);
        ...
        fn verify_read_from_cache(
            ...
            assert(cached_header.header_tag == actual_header.header_tag);
        ...
        fn verify_read_fault(grid: *const Grid, read: *const Read) void {
            ...
            if (grid.superblock.storage.grid_block(read.address)) |actual_block| {
                const actual_header = schema.header_from_block(actual_block);
                if (actual_header.header_tag == read.checksum) {
