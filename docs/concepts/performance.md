...
sport car gives you the best latency, but when the load and contention increase, due to
...
new use-cases. An OLAP database might be enough to do nightly settlement, but with OLTP real-time
settlement is a no-brainer. If a transaction system just hits its throughput target, 
every unexpected delay or an ops accident leads to missed transactions. If a system operates at one
