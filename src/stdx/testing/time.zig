...
/// These are process-specific, only counting, for example, cycles in the current process.
/// The closest clock implementation semantics are provided by CLOCK_MONOTONIC,
...
/// BenchmarkTimer is used to test algorithm runtime and is not critical for safety.
/// We still guard against non-monotonicity (e.g., https://bugzilla.redhat.com/show_bug.cgi?id=448449)
...
pub fn monotonic(self: *BenchmarkTimer) Instant {
