...
fn emit_struct(
    ...
    inline for (type_info.fields) |field| {
        ...
        switch (@typeInfo(field.type)) {
            .array => |array| try writer.print("[{d}]", .{array.len}),
            else => {},
        }
...
pub fn main() !void {
    ...
    inline for (type_mappings) |type_mapping| {
        ...
        switch (@typeInfo(ZigType)) {
            .@"struct" => |info| switch (info.layout) {
                ...
                .@"packed" => try emit_enum(&buffer.writer, ZigType, info, c_name, &.{"padding"}),
                .@"extern" => try emit_struct(&buffer.writer, info, c_name),
            },
            .@"enum" => |info| {
                ...
                try emit_enum(&buffer.writer, ZigType, info, c_name, skip);
            },
    ...
    try std.fs.File.stdout().writeAll(buffer.written());
}
