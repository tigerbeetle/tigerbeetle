...
TigerBeetle is designed specifically for [OLTP](./oltp.md) workloads. 
...
SQL. This is the power of an interface for performance!
...
- TigerBeetle is written in [Zig](https://ziglang.org/), a systems programming language which doesn't 
use garbage collection and is designed for writing fast code.
...
- TigerBeetle is designed for [io_uring](https://en.wikipedia.org/wiki/Io_uring), a Linux kernel interface 
for zero syscall networking and storage I/O.
...
It depends on the use-case (worth keeping in mind is that higher performance can _unlock_ 
new use-cases). An OLGP database might be enough for nightly settlement; for **real-time 
settlement**, OLTP is a no-brainer. 

If a transaction system just hits its throughput target, every unexpected delay or ops accident 
will lead to missed transactions. If a system operates at one tenth of capacity, there is headroom 
for the unexpected.

Last but not least, it is prudent to think about the future. The future is hard to predict (even the
_present_ is hard to wrap one's head around!); the option to handle significantly more load on
short notice greatly expands optionality and sleep quality.
...
lost, and **high availability** to ensure that money is not lost due to database downtime. 

Next, how TigerBeetle ensures [safety](./safety.md).
