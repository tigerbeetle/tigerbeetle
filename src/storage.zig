...
test "storage with mocked io layer" {
    const IO = @import("testing/io.zig").IO;
    const StorageWithMockedIO = Storage(IO);
    ...
    for (0..iterations) |_| {
        ...
        storage.write_sectors(
            struct {
                fn callback(completion: *StorageWithMockedIO.Write) void {
                    _ = completion;
                }
            }.callback,
            &write_completion,
    ...
    for (zones) |zone| {
        ...
        for (read_details[0..read_detail_length]) |read_detail| {
            ...
            storage.read_sectors(
                struct {
                    fn callback(completion: *StorageWithMockedIO.Read) void {
                        _ = completion;
                    }
                }.callback,
                &read_completion,
