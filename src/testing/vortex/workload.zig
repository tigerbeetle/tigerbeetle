...
const Result = union(Command) {
    create_accounts: []align(1) const tb.CreateAccountsResult,
    create_transfers: []align(1) const tb.CreateTransfersResult,
...
fn reconcile_(command: Command, request: Request, result: Result, model: *Model) !void {
    ...
    switch (result) {
        .create_accounts => |entries| {
            const accounts_new = request.create_accounts;
        ...
        .create_transfers => |entries| {
            ...
            for (transfers, transfers_results[0..transfers.len]) |transfer, transfer_result| {
...
pub const Model = struct {
    ...
    pub fn reconcile(
        ...
        try reconcile_(
            ...
            switch (command) {
                .create_accounts => .{
                    .create_accounts = std.mem.bytesAsSlice(tb.CreateAccountsResult, result),
                },
                .create_transfers => .{
                    .create_transfers = std.mem.bytesAsSlice(tb.CreateTransfersResult, result),
                },
                .lookup_accounts => .{
