...
pub fn TournamentTreeType(comptime Key: type, contestants_max: comptime_int) type {
    return struct {
        ...
        pub fn init(direction: Direction, contestants: []Node) TournamentTree {
            assert(contestants.len <= contestants_max);
            maybe(contestants.len == 0);
            ...
            var tree: TournamentTree = .{
                ...
                .tree_height = 0,
                .nodes_count = 0,
                .contestants_left = @intCast(contestants.len),
            };

            if (tree.contestants_left == 0) return tree;

            const leafs_count =
                std.math.ceilPowerOfTwo(u16, tree.contestants_left) catch unreachable;
            ...
            tree.contestants_left = @intCast(contestants.len);
        ...
        pub fn pop_winner(tree: *TournamentTree, new_contestant: ?Key) void {
            ...
            if (new_contestant == null) tree.contestants_left -= 1;
            tree.winner = if (new_contestant) |key|
...
pub fn KWayMergeIteratorType(
    ...
    return struct {
        ...
        pub fn init(
            ...
            for (0..streams_count) |id_usize| {
                const id: u32 = @intCast(id_usize);
                if (id > 0) assert(stream_precedence(context, id - 1, id));
            }
        ...
        fn load(self: *KWayMergeIterator) Pending!void {
            ...
            var contestants_count: u32 = 0;
            for (0..self.streams_count) |id_usize| {
                const id: u32 = @intCast(id_usize);
                const key = try stream_peek(self.context, id) orelse continue;
                contestants[id] = .{ .key = key, .id = id };
                contestants_count += 1;
            }

            self.tree = Tree.init(self.direction, contestants[0..contestants_count]);
        }
