...
pub const GridBlocksMissing = struct {
    ...
    /// Aborting queued repairs to blocks that will be freed when the current checkpoint is durable.
    pub fn checkpoint_durable_commence(
        ...
        while (faulty_blocks.next()) |fault_entry| {
            ...
            // We only need to abort repairs for blocks that are just about to be freed.
    ...
    /// Returns `true` when the `state≠waiting` faults for blocks to be freed at checkpoint
