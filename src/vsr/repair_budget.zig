...
pub const RepairBudgetJournal = struct {
    ...
    replicas_requested_prepares: []std.AutoArrayHashMapUnmanaged(u64, stdx.Instant),
    ...
    // wherein we keep requesting from apermanently crashed replica with the best repair latency.
    ...
    duration_expiry_max_ns: u64 = 500 * std.time.ns_per_ms,
    ...
    pub fn init(gpa: std.mem.Allocator, options: struct {
        ...
        for (replicas_repair_latency) |*repair_latency| {
            repair_latency.* = .{ .ns = 100 * std.time.ns_per_ms };
        }
    ...
    /// Returns the index of the replica with the lowest repair latency, and budget availability.
    /// For a fraction of ops (guided by `experiment_chance`), diverges from this heuristic and
    /// returns the index of a random replica with budget availability, using reservoir sampling.
    pub fn decrement(budget: *RepairBudgetJournal, options: struct {
        op: u64,
        requested_at: stdx.Instant,
        prng: *stdx.PRNG,
    }) ?u8 {
        ...
        for (budget.replicas_requested_prepares, 0..) |*requested_prepares, replica_index| {
            ...
            // Disallow requests to self and enforce per-replica budget.
            if (replica_index == budget.replica_index or
                requested_prepares.count() == repair_messages_inflight_count_max) continue;
        ...
        if (replica_index_maybe) |replica_index| {
            budget.replicas_requested_prepares[replica_index].putAssumeCapacity(
                options.op,
    ...
    pub fn increment(budget: *RepairBudgetJournal, options: struct {
        op: u64,
        replica_index: u8,
        repaired_at: stdx.Instant,
    }) void {
        budget.assert_invariants();
        defer budget.assert_invariants();

        // Increment budget iff we had requested this prepare.
        if (budget.replicas_requested_prepares[options.replica_index].fetchSwapRemove(
            options.op,
        )) |removed| {
            budget.available += 1;
            budget.replicas_repair_latency[options.replica_index] = ewma_add_duration(
                budget.replicas_repair_latency[options.replica_index],
                options.repaired_at.duration_since(removed.value),
            );
        }
    }
    ...
    /// for the expired requests. Penalizes the replicas for which some requests expired, by adding
    /// the duration elapsed since the request was made to their repair latency.
    ///
        ...
        for (budget.replicas_requested_prepares, 0..) |*requested_prepares, replica_index| {
            ...
            while (requested_prepares_index < requested_prepares.entries.len) {
                ...
                const duration_expiry_ns = @min(
                    2 * budget.replicas_repair_latency[replica_index].ns,
                    budget.duration_expiry_max_ns,
                );
