                ...
                switch (user_input) {
                    ...
                    .tab => {
                        ...
                        while (match_start_index > 0 and
                            !std.ascii.isWhitespace(buffer.items[match_start_index - 1]))
                        {
                            match_start_index -= 1;
                        }

                        match_prefix.clearRetainingCapacity();
                        try match_prefix.appendSlice(buffer.items[0..match_start_index]);

                        match_suffix.clearRetainingCapacity();
                        try match_suffix.appendSlice(buffer.items[buffer_index..]);

                        if (matches.items.len == 0) {
                            match_query.clearRetainingCapacity();
                            try match_query.appendSlice(
                                buffer.items[match_start_index..buffer_index],
                            );
                        ...
                        try buffer.appendSlice(match_prefix.items);
                        try buffer.appendSlice(current_match);
                        try buffer.appendSlice(match_suffix.items);
                        buffer_index = buffer.items.len - match_suffix.items.len;
                    },
