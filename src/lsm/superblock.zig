...
pub fn SuperBlock(comptime Storage: type) type {
    return struct {
        ...
        fn write_manifest(superblock: *SuperBlockGeneric, context: *Context) void {
            ...
            const offset = superblock_size * context.copy + @sizeOf(SuperBlockSector);
        ...
        fn read_sector_callback(read: *Storage.Read) void {
            ...
            if (context.copy == superblock_copies_max - 1) {
                ...
                if (superblock.quorums.working(superblock.reading, threshold)) |working| {
                    ...
                    log.debug(
                        "{s}: installed working superblock: checksum={x} sequence={} " ++
                            "commit_min={} commit_max={} view_normal={} view={}",
                        .{
                            @tagName(context.caller),
...
const TestRunner = struct {
    ...
    fn format(runner: *TestRunner) void {
        runner.pending += 1;
        runner.superblock.format(format_callback, &runner.context_format, .{
            .cluster = 123,
            .replica = 45,
...
pub fn main() !void {
    ...
    superblock.working.* = .{
        ...
        .size = size,
        .size_max = size,
        .parent = 154142934412995602786082523302531048076,
        .manifest_checksum = 0,
