...
pub fn ScanEvaluatorType(
    comptime Tree: type,
    comptime Storage: type,
    comptime EvaluatorContext: type,
    /// Decides whether to exclude a value or stop scanning.
    /// Useful to implement custom logic (e.g. expired transfers)
    /// and inequalities that otherwise would require multiple scans.
    /// e.g. `id ≠ 1` would be expressed as `id > 0 minus id = 1`.
    comptime value_next: fn (
        context: EvaluatorContext,
        value: *const Tree.Table.Value,
    ) callconv(.Inline) ValueNext,
        ...
        pub fn read(scan: *Scan, context: *Context) void {
            const on_read_callback = struct {
                fn callback(ctx: *Context, ptr: *ScanTree) void {
                    const parent: *Scan = @fieldParentPtr(Scan, "scan_tree", ptr);
