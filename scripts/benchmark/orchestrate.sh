...
echo "Spinning up replica EC2 instances - ${REPLICA_COUNT}..."
...
for replica in $(seq 1 "${REPLICA_COUNT}"); do
	...
	cat tigerbeetle.hcl | sed "s/__JOB_NAME__/${JOB_NAME}/g" | nomad job run -detach -
done

# Spin up EC2 instance for test client
echo "Spinning up client EC2 instance..."
output=$(aws ec2 run-instances \
	--image-id "${AMI_ID}" \
	--count 1 \
...
cat <<EOF
Test results will stream in at:
https://grafana.arewe1mtpsyet.com/d/5_Y-UcE4z/test-dashboard?orgId=1&from=${epoch_ms}&to=now-30s&var-test_id=${TEST_ID}
EOF