...
pub fn ReplicaType(
    ...
    return struct {
        ...
        release_execute: *const fn (replica: *Self, release: u16) void,
        ...
        ///
        /// Invariants:
        /// - upgrade_release > release_running
        ...
        /// - upgrade_targets[*].releases > release_running
        ...
        pub fn open(self: *Self, parent_allocator: std.mem.Allocator, options: OpenOptions) !void {
            ...
            assert(options.release_running >= options.release_client_min);
            ...
            try self.init(allocator, .{
                ...
                .release_running = options.release_running,
            ...
            if (release_target != self.release_running) {
        ...
        fn init(self: *Self, allocator: Allocator, options: Options) !void {
            ...
            log.debug("{}: init: replica_count={} quorum_view_change={} " ++
                "quorum_replication={} release={}", .{
                self.replica,
                self.replica_count,
        ...
        fn on_ping(self: *Self, message: *const Message.Ping) void {
            ...
            self.send_header_to_replica(message.header.replica, @bitCast(Header.Pong{
                ...
                .version_release = self.release_running,
            ...
            if (message.header.replica < self.replica_count) {
                    ...
                    upgrade_targets.*.?.timestamp > message.header.ping_timestamp_monotonic)
                {
                    ...
                    for (releases, 0..) |release, i| {
                        ...
                        if (release > self.release_running) {
        ...
        fn on_block(self: *Self, message: *const Message.Block) void {
            ...
            assert(message.header.version_vsr <= vsr.Version);
            maybe(message.header.version_vsr < vsr.Version);
        ...
        fn on_upgrade_timeout(self: *Self) void {
            ...
            const release_target: ?u16 = release: {
                for (self.releases_bundled.const_slice(), 0..) |release, i| {
                    ...
                    // Ignore old releases.
                    if (release <= self.release_running) break;
                    ...
                    for (self.upgrade_targets, 0..) |targets_or_null, replica| {
                        ...
                        for (targets.releases.const_slice()) |target_release| {
                            assert(target_release > self.release_running);
            ...
            if (release_target) |release_target_| {
                ...
                const upgrade = vsr.UpgradeRequest{ .version_release = release_target_ };
            ...
            if (self.status == .normal and self.primary()) {
                ...
                if (self.upgrade_release) |_| {
                        ...
                        self.release_for_next_checkpoint() == self.release_running) // FIXME remove?
                    {
                        ...
                        // have snuck in via the timeout (thus with the wrong version?)
                        self.send_request_upgrade_to_self();
                    }
                }
            }
        ...
        fn commit_op(self: *Self, prepare: *const Message.Prepare) void {
            ...
            assert(self.status == .normal or self.status == .view_change or
                (self.status == .recovering and self.solo()));
            assert(self.client_replies.writes.available() > 0);
            //assert(self.upgrade_release == null or prepare.header.operation == .upgrade); // FIXME remove
            ...
            //assert(self.superblock.working.vsr_state.checkpoint.release <=
            //    self.release_running);
            //if (self.superblock.working.vsr_state.checkpoint.release <
            //    self.release_running)
            //{
            ...
            if (self.primary_index(self.view) == self.replica and reply.header.client != 0) {
        ...
        fn commit_upgrade(
            ...
            assert(request.version_release >= self.release_running);
            assert(self.upgrade_release == null or
                self.upgrade_release.? == request.version_release);

            if (request.version_release == self.release_running) {
        ...
        fn ignore_request_message(self: *Self, message: *Message.Request) bool {
            ...
            if (message.header.version_release < self.release_client_min) {
                log.warn("{}: on_request: ignoring invalid version (client={} version={}<{})", .{
                    self.replica,
                    message.header.client,
                    message.header.version_release,
                    self.release_client_min,
                });
                self.send_eviction_message_to_client(message.header.client, .version_too_low);
                return true;
            }
        ...
        fn ignore_request_message_upgrade(self: *Self, message: *const Message.Request) bool {
            ...
            if (message.header.operation == .upgrade) {
                ...
                if (upgrade_request.version_release == self.release_running) {
        ...
        fn ignore_request_message_duplicate(self: *Self, message: *const Message.Request) bool {
            ...
            if (self.client_sessions.get(message.header.client)) |entry| {
                ...
                log.debug("{}: on_request: waiting for session to commit (client={})", .{
                    self.replica,
                    message.header.client,
                });
                return true;
            } else if (message.header.client == 0) {
        ...
        fn ignore_request_message_preparing(self: *Self, message: *const Message.Request) bool {
            ...
            if (self.pipeline.queue.message_by_client(message.header.client)) |pipeline_message| {
                ...
                if (message.header.client == 0) {
                    assert(message.header.operation == .upgrade);
                    log.debug("{}: on_request: ignoring (already queued upgrade)", .{self.replica});
                    return true;
                }
        ...
        fn primary_pipeline_prepare(self: *Self, request: Request) void {
            ...
            message.header.* = Header.Prepare{
                ...
                .view = self.view,
                .version_release = request_header.version_release,
                .command = .prepare,
                .replica = self.replica,
        ...
        fn repair_header(self: *Self, header: *const Header.Prepare) bool {
            ...
            if (header.op < self.op_repair_min()) {
                ...
                log.debug(
                    "{}: repair_header: op={} checksum={} false (precedes op_repair_min={})",
        ...
        fn send_message_to_replica_base(self: *Self, replica: u8, message: *Message) void {
            ...
            assert(message.header.cluster == self.cluster);
            assert(message.header.version_release <= self.release_running);
        ...
        fn release_transition(self: *Self, source: SourceLocation) void {
            const release_target = self.superblock.working.vsr_state.checkpoint.release;
            assert(release_target != self.release_running);

            // FIXME
            //std.debug.print("{}: release_transition: release={}..{} (reason={s}) journal={s}\n", .{
            //    self.replica,
            //    self.release_running,
            //    release_target,
            //    source.fn_name,
            //    @tagName(self.journal.status),
            //});

            if (self.release_running > release_target) {
                ...
                assert(self.commit_min == self.op_checkpoint());
                assert(self.release_running == self.releases_bundled.get(0));
            ...
            if (self.release_running < release_target) {
                ...
                maybe(self.release_running == self.releases_bundled.get(0));
        ...
        fn send_commit(self: *Self) void {
            ...
            if (self.primary_abdicating) {
                ...
                log.debug("{}: send_commit: primary abdicating (view={})", .{
                    self.replica,
                    self.view,
                });
                return;
