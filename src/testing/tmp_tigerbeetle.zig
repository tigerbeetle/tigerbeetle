...
//
// StreamReader echoes process' stderr unless explicitly instructed otherwise. Because
...
pub fn init(
    ...
    errdefer {
        ...
        if (reader_maybe) |reader| {
            reader.stop(gpa); // Will log stderr.
        }
    }
...
const StreamReader = struct {
    ...
    pub fn stop(self: *StreamReader, gpa: std.mem.Allocator) void {
        self.thread.join();
        gpa.destroy(self);
    }
