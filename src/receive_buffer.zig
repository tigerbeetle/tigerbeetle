...
/// ReceiveBuffer is the interface between a MessageBus and a Replica for passing batches of
/// messages while minimizing copies. It handles message framing, but doesn't do IO directly.
pub const ReceiveBuffer = struct {
    ...
    pub fn init(pool: *MessagePool) ReceiveBuffer {
    ...
    pub fn recv_slice(buffer: *ReceiveBuffer) []u8 {
    ...
    pub fn recv_advance(buffer: *ReceiveBuffer, size: u32) void {
    ...
    fn validate(buffer: *ReceiveBuffer) void {
    ...
    fn validate_header(buffer: *ReceiveBuffer) void {
        ...
        // NB: Avoid materializing invalid Zig enum value.
    ...
    fn copy_header(buffer: *const ReceiveBuffer) Header {
        assert(buffer.available_size() >= @sizeOf(Header));
    ...
    pub fn consume_message(buffer: *ReceiveBuffer, pool: *MessagePool) ?*Message {
...
test "ReceiveBuffer fuzz" {
    // Generate a byte buffer with a bunch of prepares side-by-side.
    // Optionally corrupt a single bit in the buffer.
    // Feed the buffer in chunks of varying length to the ReceiveBuffer, verify that all messages
    ...
    for (0..100) |_| {
        ...
        var receive_buffer = ReceiveBuffer.init(&pool);
        defer receive_buffer.deinit(&pool);

        var recv_size: u32 = 0;
        var recv_message_index: u32 = 0;
        while (recv_size < total_size) {
            const recv_slice = receive_buffer.recv_slice();
            ...
            receive_buffer.recv_advance(chunk_size);
            recv_size += chunk_size;

            while (receive_buffer.consume_message(&pool)) |message| {
                defer pool.unref(message);
            ...
            if (receive_buffer.invalid != null) {
                assert(fault);
                break;
            }
        }
        if (fault) {
            assert(receive_buffer.invalid != null);
        } else {
            assert(receive_buffer.invalid == null);
