...
const JVQuorumMessages = [constants.replicas_max]?*Message.JoinView;
const jv_quorum_messages_null: JVQuorumMessages = @splat(null);
...
pub fn ReplicaType(
    ...
    return struct {
        ...
        /// Probability with which the primary unconditionally injects stalls.
        commit_stall_probability: Ratio,

        /// Minimum lag in commit_min after which the primary starts injecting stalls.
        commit_stall_lag_min: u32,

        /// Maximum lag in commit_min after which the primary stops injecting stalls,
        /// considering a backup dead/partitioned.
        commit_stall_lag_max: u32,

        /// Maximum stall multiple, which puts a cap on the stall duration
        /// the primary can inject (`commit_stall_multiple_max * 10` ms).
        commit_stall_multiple_max: u16,

        /// Maximum commit_min on each backup, tracked via prepare_ok messages.
        commit_mins: [constants.replicas_max]u64 = @splat(0),

        /// Maximum head op on each backup, tracked via prepare_ok messages.
        head_ops: [constants.replicas_max]u64 = @splat(0),

        /// A random integer generated by a crypto rng during replica process startup.
        ...
        /// Unique exit_view messages for the same view from ALL replicas (including
        /// ourself).
        exit_view_from_all_replicas: QuorumCounter = quorum_counter_null,
        ...
        /// - Backups use it to trigger exit_view messages.
        /// - Primary uses it to inject extra smoothing commit messages when requests stop abruptly.
        commit_fault: vsr.FaultDetector,

        /// The number of ticks before resetting the EV quorum.
        /// (status=normal|view-change, EV quorum contains message from ANY OTHER replica)
        exit_view_window_timeout: Timeout,

        /// The number of ticks before resending a `exit_view` message.
        /// (status=normal|view-change)
        exit_view_message_timeout: Timeout,

        /// The number of ticks before a view change is timed out.
        /// When triggered, begin sending EV messages (to attempt to increment the view and try a
        /// different primary) — but keep trying JVs as well.
        /// (status=view-change)
        view_change_status_timeout: Timeout,

        /// The number of ticks before resending a `join_view` message:
        /// (status=view-change)
        ...
        /// Seeded with the replica's random_nonce.
        prng: stdx.PRNG,
        ...
        pub fn open(
            ...
            try self.init(
                ...
                .{
                    ...
                    .commit_stall_lag_min = options.commit_stall_lag_min,
                    .commit_stall_lag_max = options.commit_stall_lag_max,
            ...
            // In this case, the replica transitions to .recovering_head and waits for a .view
            ...
            if (self.log_view == self.view) {
                ...
                // recovering with a durable JV (though we still resort to this if there are no
            ...
            for (view_headers.slice) |*vsr_header| {
                if (vsr.Headers.jv_header_type(vsr_header) == .valid and
            ...
            if (self.solo()) {
                ...
                if (self.log_view == self.view) {
                    ...
                    // head op. We used only view_headers, and a JV always has a certain head op.
                    assert(self.view > self.log_view);
        ...
        fn init(
            ...
            const commit_stall_lag_min = options.commit_stall_lag_min orelse
                constants.pipeline_prepare_queue_max;
            const commit_stall_lag_max = options.commit_stall_lag_max orelse
                3 * constants.vsr_checkpoint_ops;
            const commit_stall_multiple_max = options.commit_stall_multiple_max orelse
                4;
            ...
            self.* = .{
                ...
                .commit_stall_probability = options.commit_stall_probability orelse
                    stdx.PRNG.ratio(2, 5),
                .commit_stall_lag_min = commit_stall_lag_min,
                ...
                .exit_view_window_timeout = Timeout{
                    .name = "exit_view_window_timeout",
                    .id = replica_index,
                    .after = 5_000 / constants.tick_ms,
                },
                .exit_view_message_timeout = Timeout{
                    .name = "exit_view_message_timeout",
                    .id = replica_index,
                    .after = 500 / constants.tick_ms,
                },
                .view_change_status_timeout = Timeout{
                ...
                .join_view_message_timeout = Timeout{
                    .name = "join_view_message_timeout",
                    .id = replica_index,
                    .after = 500 / constants.tick_ms,
                },
                .get_view_message_timeout = Timeout{
                ...
                .prng = stdx.PRNG.from_seed(@truncate(options.random_nonce)),
        ...
        pub fn tick(self: *Replica) void {
            ...
            const timeouts = .{
                ...
                .{ &self.exit_view_window_timeout, on_exit_view_window_timeout },
        ...
        fn on_prepare_ok(self: *Replica, message: *Message.PrepareOk) void {
            ...
            // Crucial to track commit_min for prepares outside the pipeline;
            // this message may be from a lagging replica that was withholding
            // prepare_oks (see `send_prepare_oks_after_checkpoint`).
            // Also important for the case where backups are geographically
            // far from the primary (for example multi-region deployments).
            // In that case, prepare_ok for a prepare may arrive *after* it is
            // outside of the primary's pipeline.
            self.commit_mins[message.header.replica] = @max(
                message.header.commit_min,
                self.commit_mins[message.header.replica],
            );
            self.head_ops[message.header.replica] = @max(
                message.header.op,
                self.head_ops[message.header.replica],
            );
            ...
            // This is the first time we're receiving this prepare_ok, the
            // commit_min in the message is guaranteed to be more up-to-date.
            self.commit_mins[message.header.replica] = message.header.commit_min;
        ...
        fn on_exit_view(self: *Replica, message: *Message.ExitView) void {
            assert(message.header.command == .exit_view);
            if (self.ignore_exit_view_message(message)) return;
            ...
            self.exit_view_from_all_replicas.set(message.header.replica);

            if (self.replica != message.header.replica and
                !self.exit_view_window_timeout.ticking)
            {
                self.exit_view_window_timeout.start();
            }

            const count = self.exit_view_from_all_replicas.count();
            assert(count <= threshold);
        ...
        ///
        /// When the new primary receives a quorum of join_view messages from different
        ...
        /// that new view in view-change status and begins to broadcast its own JV.
        fn on_join_view(self: *Replica, message: *Message.JoinView) void {
            assert(message.header.command == .join_view);
            ...
            assert(message.header.view == self.view);
            JVQuorum.verify_message(message);

            self.primary_receive_join_view(message);

            // Wait until we have a quorum of messages (including ourself):
            assert(self.join_view_from_all_replicas[self.replica] != null);
            assert(self.join_view_from_all_replicas[self.replica].?.header.checkpoint_op <=
                self.op_checkpoint());
            JVQuorum.verify(self.join_view_from_all_replicas);

            // Store in a var so that `.complete_valid` can capture a mutable pointer in switch.
            var headers = JVQuorum.quorum_headers(
            ...
            const op_checkpoint_max =
                JVQuorum.op_checkpoint_max(self.join_view_from_all_replicas);

            // self.commit_max could be more up-to-date than the commit_max in our JV headers.
            ...
            // commit_max. We could restart and use these view_headers as JV headers.
            const commit_max = @max(
                self.commit_max,
                JVQuorum.commit_max(self.join_view_from_all_replicas),
            );
                    ...
                    (self.view - self.log_view < self.replica_count)))
            {
                ...
                // been replaced. If we participate in another JV (before reaching status=normal,
        ...
        // their client table.
        fn on_view(self: *Replica, message: *Message.View) void {
            assert(message.header.command == .view);
            if (self.ignore_view_change_message(message.base_const())) return;

            if (self.status == .recovering_head) {
                if (message.header.view > self.view or
                    message.header.op >= self.op_prepare_max() or
                    message.header.nonce == self.random_nonce)
                {
            ...
            if (self.on_view_set_checkpoint(message)) {
                switch (self.syncing) {
                    ...
                    .canceling_commit, .canceling_grid => {
                        assert(self.sync_view == message);
                    },
                }
            } else {
                self.on_view_set_journal(message);
            }
        }
        ...
        fn on_view_set_journal(self: *Replica, message: *const Message.View) void {
            ...
            const view_headers = view_message_headers(message);
            ...
            self.view_headers.replace(.view, view_headers);
        ...
        fn on_get_view(
            ...
            const view_message = self.create_view_message(message.header.nonce);
            defer self.message_bus.unref(view_message);

            assert(view_message.header.command == .view);
            assert(view_message.references == 1);
            assert(view_message.header.view == self.view);
            assert(view_message.header.op == self.op);
            assert(view_message.header.commit_max == self.commit_max);
            assert(view_message.header.nonce == message.header.nonce);
            self.send_message_to_replica(message.header.replica, view_message);
        }
        ...
        fn on_exit_view_window_timeout(self: *Replica) void {
            assert(self.status == .normal or self.status == .view_change);
            assert(!self.exit_view_from_all_replicas.empty());
            assert(!self.solo());
        ...
        fn on_exit_view_message_timeout(self: *Replica) void {
            assert(self.status == .normal or self.status == .view_change);
            self.exit_view_message_timeout.reset();

            if (self.solo()) return;
            if (self.standby()) return;

            if (self.exit_view_from_all_replicas.is_set(self.replica)) {
                self.send_exit_view();
            }
        }
        ...
        fn on_join_view_message_timeout(self: *Replica) void {
            assert(self.status == .view_change);
            assert(!self.solo());
            self.join_view_message_timeout.reset();

            if (self.primary_index(self.view) == self.replica and self.join_view_quorum) {
                // A primary in status=view_change with a complete JV quorum must be repairing —
        ...
        fn primary_receive_join_view(
            self: *Replica,
            message: *Message.JoinView,
        ) void {
            assert(!self.solo());
            assert(self.status == .view_change);
            assert(self.primary_index(self.view) == self.replica);
            assert(self.join_view_from_all_replicas.len == constants.replicas_max);
            ...
            if (self.join_view_from_all_replicas[message.header.replica]) |m| {
                ...
                assert(message.header.replica != self.replica);
                // A replica may resend a `join_view` with a different checkpoint or commit
                        ...
                        m.header.commit_min < message.header.commit_min))
                {
                    ...
                    // TODO(Buggify): skip updating the JV, since it isn't required for
                    // correctness.
                    self.message_bus.unref(m);
                    self.join_view_from_all_replicas[message.header.replica] = message.ref();
                ...
                assert(self.join_view_from_all_replicas[message.header.replica] == null);
                self.join_view_from_all_replicas[message.header.replica] = message.ref();
            }
        }
        ...
        fn commit_stall(self: *Replica) enum { ready, pending } {
            ...
            const commit_lag = commit_lag_max: {
                ...
                for (
                    ...
                    // certain that the lagging replica can't use WAL repair
                    // and must use state sync (see `on_view_set_checkpoint`).
                    if (self.commit_min -| commit_min <= self.commit_stall_lag_max) {
                        max = @max(max, op -| commit_min);
                    }
                }
                break :commit_lag_max max;
            };
            ...
            const stall_ms = ms: {
                if (commit_lag < self.commit_stall_lag_min) {
                    while (pipeline_iterator.next()) |queue_prepare| {
                        if (queue_prepare.ok_from_all_replicas.count() >= self.quorum_replication) {
                            break;
                        }
                    } else {
                        // When there are no other committed prepares in the
                        // pipeline, backups naturally get some breathing room
                        // i.e. they get a stall by virtue of the primary not
                        // having any more work to do.
                        break :ms 0;
                    }

                    // There are other committed prepares in the pipeline, give
                    // backups some breathing room by explicitly stalling.
                    // NB: We do this with commit_stall_probability to achieve
                    // shorter than a tick resolution on average, *not* to
                    // randomize when stalls are injected.
                    ...
                    const checkpoint_ratio = @divFloor(
                        constants.vsr_checkpoint_ops,
                        self.commit_stall_multiple_max,
                    );
                    const stall_multiple =
                        std.math.clamp(
                            @divFloor(commit_lag, checkpoint_ratio),
                            1,
                            self.commit_stall_multiple_max,
                        );
        ...
        fn create_view_message(self: *Replica, random_nonce: u128) *Message.View {
            assert(self.status == .normal or self.status == .view_change);
            assert(self.syncing != .updating_checkpoint);
            ...
            const message = self.message_bus.get_message(.view);
            defer self.message_bus.unref(message);
        ...
        fn update_view_headers(self: *Replica) void {
            assert(self.status != .recovering_head);
            assert(self.view == self.log_view);
            assert(self.view_headers.command == .view);

            const op_range = self.update_view_headers_op_range();
            if (self.primary_index(self.view) == self.replica) {
        ...
        fn update_view_headers_op_range(self: *const Replica) struct {
            op_min: u64,
            op_max: u64,
        } {
            assert(self.status != .recovering_head);
        ...
        fn flush_loopback_queue(self: *Replica) void {
            ...
            // We expect that delivering a prepare_ok or join_view message to ourselves will
        ...
        fn ignore_exit_view_message(
            self: *const Replica,
            message: *const Message.ExitView,
        ) bool {
            assert(message.header.command == .exit_view);
            assert(message.header.replica < self.replica_count);
        ...
        fn ignore_view_change_message(self: *const Replica, message: *const Message) bool {
            assert(message.header.command == .join_view or
                message.header.command == .view);
        ...
        fn repair(self: *Replica) void {
            ...
            const repair_op_max: u64 = repair: {
                ...
                // Missing prepares/headers within a pipeline of ops
                // from the head may arrive via normal replication, so
                // wait for them instead of eagerly repairing.
            ...
            if (self.status == .view_change and self.primary_index(self.view) == self.replica) {
                ...
                // Sending view messages to backups and committing up to commit_max can be
        ...
        fn primary_journal_repaired(self: *const Replica) bool {
            ...
            assert(self.primary_index(self.view) == self.replica);
            assert(self.view == self.log_view);
            if (self.status == .view_change) assert(self.join_view_quorum);
        ...
        fn reset_quorum_join_view(self: *Replica) void {
            self.reset_quorum_messages(&self.join_view_from_all_replicas, .join_view);
            self.join_view_quorum = false;
        }

        fn reset_quorum_exit_view(self: *Replica) void {
            self.reset_quorum_counter(&self.exit_view_from_all_replicas);
        }
        ...
        fn send_prepare_ok(self: *Replica, header: *const Header.Prepare) void {
            ...
            if (self.journal.has_prepare(header)) {
                ...
                // the new primary doesn't learn about in the join_view messages.
        ...
        fn send_exit_view(self: *Replica) void {
            assert(self.status == .normal or self.status == .view_change);
            assert(!self.solo());

            if (self.standby()) return;

            const header = Header.ExitView{
                .command = .exit_view,
                .cluster = self.cluster,
                .replica = self.replica,
                .view = self.view,
        ...
        fn send_join_view(self: *Replica) void {
            ...
            assert(self.log_view >= self.log_view_durable());
            assert(!self.join_view_quorum);
            // The JV headers are already up to date, either via:
            // - transition_to_view_change_status(), or
            // - superblock's view_headers (after recovery).
            assert(self.view_headers.command == .join_view);
            assert(self.view_headers.array.get(0).op >= self.op);
            self.view_headers.verify();

            const BitSet = stdx.BitSetType(128);
            comptime assert(BitSet.Word ==
                @FieldType(Header.JoinView, "present_bitset"));
            comptime assert(BitSet.Word ==
                @FieldType(Header.JoinView, "nack_bitset"));
            ...
            //   JV messages to decide whether to keep or truncate the header.
            var nacks: BitSet = .{};
            var present: BitSet = .{};
            for (self.view_headers.array.const_slice(), 0..) |*header, i| {
                ...
                if (vsr.Headers.jv_header_type(header) == .valid) {
                    ...
                    assert(vsr.Headers.jv_header_type(header) == .blank);
                    assert(!present.is_set(i));
                    if (nacks.is_set(i)) assert(!faulty);
                }
            }

            const message = self.message_bus.get_message(.join_view);
            defer self.message_bus.unref(message);
            ...
            if (self.replica == self.primary_index(self.view) and
                self.join_view_from_all_replicas[self.replica] == null)
            {
        ...
        fn send_message_to_replica_base(self: *Replica, replica: u8, message: *Message) void {
            ...
            if (replica != self.replica and message.header.replica == self.replica) {
                    ...
                    message.header.command == .prepare_ok)
                {
                    ...
                    assert(message.header.command != .join_view or std.mem.eql(
                        u8,
                        message.body_used(),
        ...
        /// A replica must not advertise a log_view (in a JV) higher than its durable log_view.
        ///
        /// A replica's advertised `log_view` must never backtrack after a crash.
        /// (`log_view` is only advertised within JV messages).
        ///
        ...
        /// If replica 2 is partitioned, replicas 0 and 1 start view d with the JVs:
        ///
        ...
        fn primary_send_view(self: *Replica) void {
            assert(self.status == .normal or self.status == .view_change);
            assert(self.replica == self.primary_index(self.view));
            ...
            const view_message = self.create_view_message(nonce);
            defer self.message_bus.unref(view_message);

            assert(view_message.header.command == .view);
            assert(view_message.header.nonce == 0);

            self.send_message_to_other_replicas(view_message);
        }
        ...
        /// Load the new view's headers from the JV quorum.
        ///
        /// The iteration order of JVs for repair does not impact the final result.
        /// In other words, you can't end up in a situation with a JV quorum like:
        ///
        ///   replica     headers  commit_min
        ///         0   4 5 _ _ 8           4 (new primary; handling JV quorum)
        ///         1   4 _ 6 _ 8           4
        ///         2   4 _ _ 7 8           4
        ///         3  (4 5 6 7 8)          8 (didn't participate in view change)
        ///         4  (4 5 6 7 8)          8 (didn't participate in view change)
        ///
        /// where the new primary's headers depends on which of replica 1 and 2's JV is used
        ...
        fn primary_set_log_from_join_view_messages(self: *Replica) void {
            assert(self.status == .view_change);
            assert(self.view > self.log_view);
            assert(self.primary_index(self.view) == self.replica);
            assert(!self.solo());
            ...
            assert(jvs_all.count() >= self.quorum_view_change);

            for (jvs_all.const_slice()) |message| {
            ...
            const timestamp_max = JVQuorum.timestamp_max(self.join_view_from_all_replicas);
            if (self.state_machine.prepare_timestamp < timestamp_max) {
                self.state_machine.prepare_timestamp = timestamp_max;
            }

            var quorum_headers = JVQuorum.quorum_headers(
                self.join_view_from_all_replicas,
                .{
                    .quorum_nack_prepare = self.quorum_nack_prepare,
                    .quorum_view_change = self.quorum_view_change,
            ...
            const commit_max = JVQuorum.commit_max(self.join_view_from_all_replicas);
            ...
            const jvs_uncanonical =
                JVQuorum.jvs_uncanonical(self.join_view_from_all_replicas);
            for (jvs_uncanonical.const_slice()) |message| {
                const message_headers = message_body_as_view_headers(message.base_const());
                for (message_headers.slice) |*header| {
                    if (vsr.Headers.jv_header_type(header) != .valid) continue;
        ...
        fn primary_log_join_view_quorum(
            self: *const Replica,
        ...
        fn transition_to_recovering_head_from_recovering_status(self: *Replica) void {
            ...
            assert(!self.prepare_timeout.ticking);
            assert(!self.primary_abdicate_timeout.ticking);
            assert(!self.exit_view_message_timeout.ticking);
            assert(!self.exit_view_window_timeout.ticking);
        ...
        fn transition_to_normal_from_view_change_status(self: *Replica, view_new: u32) void {
            ...
            self.reset_quorum_exit_view();
            self.reset_quorum_join_view();

            // Err on the side of not injecting commit stalls as a new primary,
            // as opposed to using potentially outdated data to inject stalls
            // (see `commit_stall`).
            self.commit_mins = @splat(0);
            self.head_ops = @splat(0);

            assert(self.join_view_quorum == false);
        }
        ...
        fn transition_to_view_change_status(self: *Replica, view_new_min: u32) void {
            ...
            assert(view_new_min > self.log_view);
            assert(self.commit_max >= self.op -| constants.pipeline_prepare_queue_max);
            defer assert(self.view_headers.command == .join_view);
            ...
            self.join_view_message_timeout.start();
        ...
        fn update_join_view_headers(self: *Replica) void {
            // Either:
            ...
            // might belong to the next log wrap), to guarantee that a JV with log_view=v includes
            // all uncommitted ops with views <v. Special case: a primary that has collected a JV
        ...
        fn sync_cancel_grid_callback(grid: *Grid) void {
            ...
            assert(message.header.command == .view);
            const checkpoint = view_message_checkpoint(message);
        ...
        fn jump_view(self: *Replica, header: *const Header) void {
            ...
            const to: Status = switch (header.command) {
                ...
                // But there is a chance that the primary is actually running, despite the JV/EV.
                .join_view,
                .exit_view,
                // For pings, we don't actually know where the new view is started or not.
                // Conservatively transition to view change: at worst, we'll send a larger JV
                // instead of a RV.
                .ping,
                .pong,
                => if (self.status == .recovering_head) Status.normal else .view_change,
                // on_view() handles the (possible) transition to view-change manually, before
        ...
        fn write_prepare(self: *Replica, message: *Message.Prepare) bool {
            ...
            assert(self.status == .normal or self.join_view_quorum);
...
/// - *blank*: A header that explicitly marks a gap in the JV headers.
///   (See `vsr.Headers.jv_blank()`).
...
/// - A *canonical message* is any JV message from the most recent log_view in the quorum.
...
///
///
/// Invariants (for each JV message):
///
...
///   - The JV-sender is responsible for ensuring blanks do not conceal chain breaks.
///   - For example,
///     - a JV of 6a,7_,8a is valid (6a/8a belong to the same chain).
///     - a JV of 6b,7_,8a is invalid (the gap at 7 conceal a chain break).
///     - a JV of 6b,7b,8a is invalid (7b/8a is a chain break)..
/// - All pipeline headers present on the replica must be included in the JV headers.
...
///
/// Each header in the JV body is one of:
///
...
///
/// Invariants (across all JVs in the quorum):
///
...
const JVQuorum = struct {
    ...
    fn verify(jv_quorum: JVQuorumMessages) void {
        ...
        for (jvs.const_slice(), 0..) |jv_a, i| {
            for (jvs.const_slice()[0..i]) |jv_b| {
                ...
                const headers_b = message_body_as_view_headers(jv_b.base_const());
                // Find the intersection of the ops covered by each JV.
                const op_max = @min(jv_a.header.op, jv_b.header.op);
                const op_min = @max(
                ...
                while (op <= op_max) : (op += 1) {
                    const header_a = &headers_a.slice[jv_a.header.op - op];
                    const header_b = &headers_b.slice[jv_b.header.op - op];
    ...
    fn verify_message(message: *const Message.JoinView) void {
        assert(message.header.command == .join_view);
    ...
    fn jvs_all(jv_quorum: JVQuorumMessages) JVArray {
        var array = JVArray{};
        for (jv_quorum, 0..) |received, replica| {
            if (received) |message| {
                assert(message.header.command == .join_view);
                assert(message.header.replica == replica);
    ...
    fn jvs_canonical(jv_quorum: JVQuorumMessages) JVArray {
        return jvs_with_log_view(jv_quorum, JVQuorum.log_view_max(jv_quorum));
    }
    ...
    /// Returns the highest `log_view` of any JV.
    ///
    /// The headers bundled with JVs with the highest `log_view` are canonical, since
    ...
    /// - Return an iterator over the canonical JV's headers, from high-to-low op.
    ///   The first header returned is the new head message.
    /// Otherwise:
    /// - Return the reason the view cannot begin.
    fn quorum_headers(jv_quorum: JVQuorumMessages, options: struct {
        quorum_nack_prepare: u8,
        quorum_view_change: u8,
        replica_count: u8,
    }) union(enum) {
        // The quorum has fewer than "quorum_view_change" JVs.
        // We are waiting for JVs from the remaining replicas.
        awaiting_quorum,
        // The quorum has at least "quorum_view_change" JVs.
        // The quorum has fewer than "replica_count" JVs.
        // The quorum collected so far is insufficient to determine which headers can be nacked
        // (due to an excess of faults).
        // We must wait for JVs from one or more remaining replicas.
        awaiting_repair,
        // All replicas have contributed a JV, but there are too many faults to start a new view.
        ...
        const jvs_all_ = JVQuorum.jvs_all(jv_quorum);
        if (jvs_all_.count() < options.quorum_view_change) return .awaiting_quorum;

        const log_view_canonical = JVQuorum.log_view_max(jv_quorum);
        const jvs_canonical_ = JVQuorum.jvs_canonical(jv_quorum);
        assert(jvs_canonical_.count() > 0);
        assert(jvs_canonical_.count() <= jvs_all_.count());

        const op_head_max = JVQuorum.op_max_canonical(jv_quorum);
        const op_head_min = JVQuorum.commit_max(jv_quorum);
        ...
        const op_head = while (op <= op_head_max) : (op += 1) {
            const header_canonical = for (jvs_canonical_.const_slice()) |jv| {
                ...
                if (vsr.Headers.jv_header_type(header) == .valid) break header;
            } else null;
            ...
            {
                if (jvs_all_.count() < options.replica_count) {
...
fn view_message_checkpoint(message: *const Message.View) *const vsr.CheckpointState {
    assert(message.header.command == .view);
...
fn view_message_headers(message: *const Message.View) []const Header.Prepare {
    assert(message.header.command == .view);
    ...
    vsr.Headers.ViewChangeSlice.verify(.{ .command = .view, .slice = headers });
    if (constants.verify) {
        for (headers) |*header| {
            assert(header.valid_checksum());
            assert(vsr.Headers.jv_header_type(header) == .valid);
        }
    }
    return headers;
}
