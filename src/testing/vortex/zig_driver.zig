const std = @import("std");
const Driver = @import("driver.zig");

const c = @cImport({
    _ = @import("../../tb_client_exports.zig"); // Needed for the @export()'ed C ffi functions.
    @cInclude("tb_client.h");
});

const assert = std.debug.assert;

const log = std.log.scoped(.zig_driver);

pub const CLIArgs = struct {
    positional: struct {
        @"cluster-id": u128,
        addresses: []const u8,
    },
};

pub fn main(_: std.mem.Allocator, args: CLIArgs) !void {
    log.info("addresses: {s}", .{args.positional.addresses});

    var tb_client: c.tb_client_t = undefined;
    const cluster_id = args.positional.@"cluster-id";
    const tb_context: usize = 42;
    const result = c.tb_client_init(
        &tb_client,
        cluster_id,
        args.positional.addresses.ptr,
        @intCast(args.positional.addresses.len),
        tb_context,
        on_complete,
    );
    if (result != c.TB_STATUS_SUCCESS) {
        return error.ClientInitError;
    }
    defer c.tb_client_deinit(tb_client);

    const stdin = std.io.getStdIn().reader().any();
    const stdout = std.io.getStdOut().writer().any();

    while (true) {
        const request = receive(stdin) catch |err| {
            switch (err) {
                error.EndOfStream => break,
                else => return err,
            }
        };

        const operation = std.meta.activeTag(request);
        switch (request) {
            inline else => |events| {
                const data = std.mem.sliceAsBytes(events);

                var packet: c.tb_packet_t = undefined;
                packet.operation = @intFromEnum(operation);
                packet.user_data = @constCast(@ptrCast(&stdout));
                packet.data = @constCast(data.ptr);
                packet.data_size = @intCast(data.len);
                packet.next = null;
                packet.status = c.TB_PACKET_OK;

                c.tb_client_submit(tb_client, &packet);
            },
        }
    }
}

pub fn on_complete(
    tb_context: usize,
    tb_client: c.tb_client_t,
    tb_packet: [*c]c.tb_packet_t,
    result_ptr: [*c]const u8,
    result_len: u32,
) callconv(.C) void {
    _ = tb_context;
    _ = tb_client;
    const writer: *std.io.AnyWriter = @ptrCast(@alignCast(tb_packet.*.user_data.?));
    const operation: Driver.Operation = @enumFromInt(tb_packet.*.operation);

    // TODO: move this off tb_client thread
    if (result_ptr) |result| {
        write_results(writer, operation, result[0..result_len]) catch |err| {
            switch (err) {
                error.BrokenPipe => {},
                else => log.err("error when writing back results: {any}", .{err}),
            }
        };
    }
}

fn write_results(writer: *std.io.AnyWriter, operation: Driver.Operation, result: []const u8) !void {
    switch (operation) {
        inline else => |comptime_operation| {
            const size = @sizeOf(Driver.Result(comptime_operation));
            if (size > 0) {
                const count = @divExact(result.len, size);
                try writer.writeInt(u32, @intCast(count), .little);
                try writer.writeAll(result);
            } else {
                log.err(
                    "unexpected size {d} for op: {s}",
                    .{ size, @tagName(comptime_operation) },
                );
            }
        },
    }
}

fn receive(reader: std.io.AnyReader) !Driver.Request {
    const operation = try reader.readEnum(Driver.Operation, .little);
    const count = try reader.readInt(u32, .little);

    return switch (operation) {
        inline else => |comptime_operation| {
            assert(count <= Driver.events_count_max);
            const buffer_size_max =
                @sizeOf(Driver.Event(comptime_operation)) * Driver.events_count_max;

            var buffer: [buffer_size_max]u8 = undefined;

            const buffer_size = @sizeOf(Driver.Event(comptime_operation)) * count;
            assert(try reader.readAtLeast(buffer[0..buffer_size], buffer_size) == buffer_size);

            const events: []const Driver.Event(comptime_operation) = @alignCast(
                std.mem.bytesAsSlice(Driver.Event(comptime_operation), buffer[0..buffer_size]),
            );

            return @unionInit(Driver.Request, @tagName(comptime_operation), events);
        },
    };
}
