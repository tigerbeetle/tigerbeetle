...
test "tidy" {
    ...
    for (paths) |path| {
        ...
        if (mem.endsWith(u8, source_file.path, ".zig")) {
            ...
            tidy_long_function(source_file, &tree);
...
fn tidy_long_function(
    ...
    for (functions[0..functions_count], 0..) |f, index| {
            ...
            functions[index + 1].line_opening > f.line_closing)
        {
            ...
            if (function_length > 70) {
                if (function_length >= 72) {
                    // Backwards ratchet: sufficiently bad functions become good, and you win a
                    // cookie if you make the number above higher!
                } else {
