...
pub const io_uring_sqe = extern struct {
    opcode: linux.IORING_OP, flags: u8, ioprio: u16, fd: i32, off: u64, addr: u64, len: u32, rw_flags: u32, user_data: u64, buf_index: u16, personality: u16, splice_fd_in: i32, __pad2: [2]u64
};
...
pub const IO_Uring = struct {
    ...
    pub fn read(self: *IO_Uring, user_data: u64, fd: os.fd_t, buffer: []u8, offset: u64) !*io_uring_sqe {
        const sqe = try self.get_sqe();
    ...
    /// Returns a pointer to the SQE.
    pub fn write(self: *IO_Uring, user_data: u64, fd: os.fd_t, buffer: []const u8, offset: u64) !*io_uring_sqe {
        const sqe = try self.get_sqe();
    ...
    pub fn readv(self: *IO_Uring, user_data: u64, fd: os.fd_t, iovecs: []const os.iovec, offset: u64) !*io_uring_sqe {
        const sqe = try self.get_sqe();
    ...
    pub fn writev(self: *IO_Uring, user_data: u64, fd: os.fd_t, iovecs: []const os.iovec_const, offset: u64) !*io_uring_sqe {
        const sqe = try self.get_sqe();
    ...
    /// Returns a pointer to the SQE.
    pub fn accept(self: *IO_Uring, user_data: u64, fd: os.fd_t, addr: *os.sockaddr, addrlen: *os.socklen_t, flags: u32) !*io_uring_sqe {
        const sqe = try self.get_sqe();
    ...
    /// Queues (but does not submit) an SQE to perform a `connect(2)` on a socket.
    /// Returns a pointer to the SQE.
    pub fn connect(self: *IO_Uring, user_data: u64, fd: os.fd_t, addr: *const os.sockaddr, addrlen: os.socklen_t) !*io_uring_sqe {
        const sqe = try self.get_sqe();
    ...
    /// Returns a pointer to the SQE.
    pub fn recv(self: *IO_Uring, user_data: u64, fd: os.fd_t, buffer: []u8, flags: u32) !*io_uring_sqe {
        const sqe = try self.get_sqe();
    ...
    /// Returns a pointer to the SQE.
    pub fn send(self: *IO_Uring, user_data: u64, fd: os.fd_t, buffer: []const u8, flags: u32) !*io_uring_sqe {
        const sqe = try self.get_sqe();
    ...
    /// Returns a pointer to the SQE.
    pub fn openat(self: *IO_Uring, user_data: u64, fd: os.fd_t, path: [*:0]const u8, flags: u32, mode: os.mode_t) !*io_uring_sqe {
        const sqe = try self.get_sqe();
    ...
    pub fn register_files(self: *IO_Uring, fds: []const os.fd_t) !void {
        ...
        const res = linux.io_uring_register(self.fd, .REGISTER_FILES, @ptrCast(*const c_void, fds.ptr), @intCast(u32, fds.len));
        switch (linux.getErrno(res)) {
            0 => {},
...
pub const SubmissionQueue = struct {
    ...
    pub fn init(fd: os.fd_t, p: io_uring_params) !SubmissionQueue {
        ...
        const size = std.math.max(p.sq_off.array + p.sq_entries * @sizeOf(u32), p.cq_off.cqes + p.cq_entries * @sizeOf(io_uring_cqe));
...
pub const CompletionQueue = struct {
    ...
    pub fn init(fd: os.fd_t, p: io_uring_params, sq: SubmissionQueue) !CompletionQueue {
        ...
        assert((p.features & linux.IORING_FEAT_SINGLE_MMAP) != 0);
        const mmap = sq.mmap;
        const cqes = @ptrCast([*]io_uring_cqe, @alignCast(@alignOf(io_uring_cqe), &mmap[p.cq_off.cqes]));
        assert(p.cq_entries ==
...
pub fn io_uring_prep_rw(op: linux.IORING_OP, sqe: *io_uring_sqe, fd: os.fd_t, addr: anytype, len: usize, offset: u64) void {
    sqe.* = .{
        .opcode = op,
        .flags = 0,
        .ioprio = 0,
...
pub fn io_uring_prep_readv(sqe: *io_uring_sqe, fd: os.fd_t, iovecs: []const os.iovec, offset: u64) void {
    io_uring_prep_rw(.READV, sqe, fd, iovecs.ptr, iovecs.len, offset);
}

pub fn io_uring_prep_writev(sqe: *io_uring_sqe, fd: os.fd_t, iovecs: []const os.iovec_const, offset: u64) void {
    io_uring_prep_rw(.WRITEV, sqe, fd, iovecs.ptr, iovecs.len, offset);
}

pub fn io_uring_prep_accept(sqe: *io_uring_sqe, fd: os.fd_t, addr: *os.sockaddr, addrlen: *os.socklen_t, flags: u32) void {
...
pub fn io_uring_prep_connect(sqe: *io_uring_sqe, fd: os.fd_t, addr: *const os.sockaddr, addrlen: os.socklen_t) void {
...
pub fn io_uring_prep_openat(sqe: *io_uring_sqe, fd: os.fd_t, path: [*:0]const u8, flags: u32, mode: os.mode_t) void {
...
test "writev/fsync/readv" {
    ...
    const iovecs_write = [_]os.iovec_const{os.iovec_const{ .iov_base = &buffer_write, .iov_len = buffer_write.len }};
    var buffer_read = [_]u8{0} ** 128;
    var iovecs_read = [_]os.iovec{os.iovec{ .iov_base = &buffer_read, .iov_len = buffer_read.len }};
