...
test "Cluster: view_change: DVC header doesn't match current header in journal" {
    ...
    // if the DVC was computed when that header was corrupt/not present in the replica's journal,
    // and the replica is simply reusing an old DVC). The replica must check the journal while
    // *sending* the DVC, so it appropriately acks/nacks headers in the DVC based on the current
