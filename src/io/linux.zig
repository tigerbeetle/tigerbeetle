    ...
    fn flush(self: *IO, wait_nr: u32, timeouts: *usize, etime: *bool) !void {
        ...
        while (self.completed.pop()) |completion| {
            if (self.awaiting) |awaiting| {
                ...
                assert(completion.operation.timeout.timespec.tv_sec == 0);
                assert(completion.operation.timeout.timespec.tv_nsec == 0);
            ...
            switch (self.cancel_status) {
                ...
                .wait => |wait| if (wait.target == completion) { self.cancel_status = .next; },
                .done => unreachable,
            }
        }
