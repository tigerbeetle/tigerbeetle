...
pub fn ReplType(comptime MessageBus: type) type {
    ...
    return struct {
                ...
                switch (user_input) {
                    ...
                    .tab => {
                        ...
                        while (match_start_index > 0 and
                            !std.ascii.isWhitespace(repl.buffer.items[match_start_index - 1]))
                        {
                            match_start_index -= 1;
                        }

                        match_prefix.clearRetainingCapacity();
                        try match_prefix.appendSlice(repl.buffer.items[0..match_start_index]);

                        match_suffix.clearRetainingCapacity();
                        try match_suffix.appendSlice(repl.buffer.items[buffer_index..]);

                        if (matches.items.len == 0) {
                            match_query.clearRetainingCapacity();
                            try match_query.appendSlice(
                                repl.buffer.items[match_start_index..buffer_index],
                            );
                        ...
                        repl.buffer.clearRetainingCapacity();
                        try repl.buffer.appendSlice(match_prefix.items);
                        try repl.buffer.appendSlice(current_match);
                        try repl.buffer.appendSlice(match_suffix.items);
                        buffer_index = repl.buffer.items.len - match_suffix.items.len;
                    },
        ...
        fn do_repl(
            ...
            const input = repl.read_until_newline_or_eof(arena.allocator()) catch |err| {
