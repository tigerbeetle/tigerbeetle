...
pub fn JournalType(comptime Replica: type, comptime Storage: type) type {
    return struct {
        ...
        /// 4. If at this point the replica crashes & restarts, slot 6 is in case `@M`
        ///    (decision=nil) which can be locally repaired.
        ///    In contrast, if op 6's prepare header was written in step 3, it would be case `@M`,
        ...
        fn recover_slots(journal: *Journal) void {
            ...
            // - after the case decisions are made (to avoid @L:vsr arising from an
        ...
        fn recover_slot(journal: *Journal, slot: Slot, case: *const Case) void {
            ...
            switch (decision) {
                ...
                .cut => {
                    ...
                    assert(prepare != null);
                    assert(prepare.?.operation != .reserved);
                    assert(prepare.?.op > replica.op_prepare_max());
                    assert(journal.prepare_inhabited[slot.index]);
                    assert(journal.prepare_checksums[slot.index] == prepare.?.checksum);
                    journal.headers[slot.index] = Header.Prepare.reserve(cluster, slot.index);
                    journal.dirty.clear(slot);
        ...
        pub fn remove_entries_from(journal: *Journal, op_min: u64) void {
            ...
            for (journal.headers, 0..) |*header, index| {
                ...
                // since a reserved header may have been marked faulty for case @L, and
...
const recovery_cases = table: {
    ...
    break :table [_]Case{
        ...
        //    op>  prepare.op/header.op > op_prepare_max
        ...
        Case.init("@H", .cut, .eql, .{ __, __, _1, _0, __, _1, __, __, __, __, __ }), // prepare.op > op_prepare_max
        Case.init("@I", .cut, .eql, .{ _1, _0, _1, _0, __, _0, _1, __, __, a0, __ }), // header.op > op_prepare_max, prepare valid
        Case.init("@J", .cut, .eql, .{ _1, _0, _1, _1, __, __, _1, __, __, a0, __ }), // header.op > op_prepare_max, prepare reserved
