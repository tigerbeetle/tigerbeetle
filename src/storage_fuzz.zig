...
pub fn main(args: fuzz.FuzzArgs) !void {
    ...
    const storage_size = sector_count * constants.sector_size;
    const iterations = args.events_max orelse 10_000;

    const failed_sector_cluster_count = 7;
    const failed_sector_cluster_minimum_length = 1;
    const failed_sector_cluster_maximum_length = 4;
    ...
    for (0..failed_sector_cluster_count) |_| {
        const start = data_prng.random().intRangeLessThan(
            usize,
            0,
            sector_count - failed_sector_cluster_maximum_length,
        );
        const end = start + data_prng.random().intRangeAtMost(
            usize,
    ...
    for (0..sector_count) |sector| {
        if (!fault_map.isSet(sector)) {
            data_prng.fill(
                storage_data_written[sector * constants.sector_size ..][0..constants.sector_size],
            );
        }
    }
    ...
    for (0..iterations) |_| {
        for (zones) |zone| {
            ...
            const zone_sector_count: u64 = @divExact(zone.size().?, constants.sector_size);
