...
pub fn ReplicaType(
    ...
    return struct {
        ...
        /// Maps replica identities to logical indexes used by the MessageBus.
        ...
        epoch: u32,
        /// True when the primary accepted, but not yet committed a reconfiguration request. Only
        /// primary tracks this, because only primary controls the order of not yet committed
        /// prepares.
        epoch_pending: bool,
        ...
        fn init(self: *Self, allocator: Allocator, options: Options) !void {
            ...
            const synchronization_disabled = replica_count == 1;
            self.clock = try Clock.init(
                allocator,
                replica_count,
                if (replica_index < replica_count) replica_index else 0,
        ...
        pub fn on_message(self: *Self, message: *Message) void {
            ...
            log.debug("{}: on_message: epoch={} view={} status={} {}", .{
                self.replica,
                self.epoch,
                self.view,
                self.status,
            ...
            if (self.ignore_message_different_epoch(message.header)) {
                log.debug("{}: on_message: ignoring, different epoch ({s})", .{
                    self.replica,
        ...
        fn on_ping_client(self: *Self, message: *const Message) void {
            ...
            assert(message.header.epoch <= self.epoch);
            maybe(message.header.epoch < self.epoch and self.epoch_old == null);
        ...
        fn on_request(self: *Self, message: *Message) void {
            ...
            assert(message.header.command == .request);
            // The client's epoch or view may be behind ours, but never ahead.
        ...
        fn on_repair(self: *Self, message: *Message) void {
            ...
            if (!vsr.view_order_or_eql(message.header, self)) {
                log.debug("{}: on_repair: ignoring (newer view)", .{self.replica});
                return;
            }

            if (self.status == .view_change and message.header.epoch == self.epoch and
            ...
            if (message.header.operation == .reconfigure and message.header.epoch == self.epoch) {
                // Reconfiguration is always the final op of an epoch.
                assert(message.header.op == self.op);
            }
        ...
        fn on_end_epoch(self: *Self, message: *const Message) void {
            ...
            self.epoch_switch_to_new(message);
        }
        ...
        fn on_request_prepare(self: *Self, message: *const Message) void {
            ...
            if (self.pipeline_prepare_by_op_and_checksum(op, checksum)) |prepare| {
                ...
                self.send_message_to_replica_epoch(
        ...
        fn on_request_prepare_read(
            self: *Self,
            prepare: ?*Message,
            destination_epoch_opt: ?u32,
            destination_replica_opt: ?u8,
        ) void {
            ...
            const destination_epoch = destination_epoch_opt.?;
            const destination_replica = destination_replica_opt.?;
            assert(destination_epoch <= self.epoch);

            if (!(destination_epoch == self.epoch or
                (self.epoch_old != null and destination_epoch == self.epoch_old.?.epoch)))
            {
                log.debug("{}: on_request_prepare_read: epoch retired (epoch={})", .{
                    self.replica,
                    destination_epoch,
                });
                return;
            ...
            log.debug("{}: on_request_prepare_read: op={} checksum={} " ++
                "sending to epoch={} replica={}", .{
                self.replica,
                message.header.op,
                message.header.checksum,
                destination_epoch,
            ...
            self.send_message_to_replica_epoch(destination_epoch, destination_replica, message);
        }

        // TODO: How should this work across epoch? Newly standby should reply only to a part of the
        // range.
        fn on_request_headers(self: *Self, message: *const Message) void {
            ...
            if (message.header.epoch != self.epoch) {
                assert(op_max <= self.epoch_old.?.final_prepare.op);
            }
            ...
            // Assert that headers are valid.
            const headers = message_body_as_headers(response);
            if (message.header.epoch != self.epoch) {
                assert(headers[0].op <= self.epoch_old.?.final_prepare.op);
                response.header.epoch = headers[0].epoch;
                response.header.view = headers[0].view;
            }
        ...
        fn on_end_epoch_message_timeout(self: *Self) void {
            ...
            stdx.copy_disjoint(
                ...
                std.mem.asBytes(&self.members),
            );

            message.header.size = @sizeOf(Header) + @sizeOf([constants.nodes_max]u128);
        ...
        fn commit_journal_next(self: *Self) void {
            ...
            if (self.commit_min < self.commit_max and self.commit_min < self.op) {
                ...
                if (self.pipeline.cache.prepare_by_op_and_checksum(op, header.checksum)) |prepare| {
                    ...
                    self.commit_journal_next_callback(prepare, null, null);
                } else {
        ...
        fn commit_journal_next_callback(self: *Self, prepare: ?*Message, destination_epoch: ?u32, destination_replica: ?u8) void {
            assert(self.committing);
            assert(destination_epoch == null);
        ...
        fn commit_op_client_replies_ready(client_replies: *ClientReplies) void {
            ...
            if (self.status == .normal and self.primary()) {
                ...
                if (self.epoch_pending) {
                    while (self.pipeline.queue.pop_request()) |request| {
                        self.message_bus.unref(request.message);
                    }
                } else {
        ...
        fn commit_op(self: *Self, prepare: *const Message) void {
            ...
            if (prepare.header.operation == .reconfigure) {
                if (self.epoch == prepare.header.epoch) {
        ...
        fn ignore_message_different_epoch(self: *const Self, header: *const Header) bool {
            // Always allow the current epoch.
            if (self.epoch == header.epoch) return false;

            // Never allow future epoch.
            if (self.epoch < header.epoch) return true;

            assert(self.epoch > header.epoch);

            // Always allow prepares from old epochs.
            if (header.command == .prepare) return false;

            // Always allow client messages from previous epochs.
            if (header.command == .ping_client or header.command == .request) return false;

            // During reconfiguration, allow cross-epoch repair.
            if (self.epoch_old != null and header.epoch == self.epoch_old.?.epoch) {
        ...
        fn ignore_repair_message(self: *Self, message: *const Message) bool {
            ...
            if (self.ignore_repair_message_during_view_change(message)) {
                return true;
            }
        ...
        fn ignore_repair_message_during_view_change(self: *Self, message: *const Message) bool {
            ...
            switch (message.header.command) {
                ...
                .request_headers, .request_prepare, .request_reply => {
                    if (message.header.epoch != self.epoch) return true;
        ...
        fn ignore_request_message_backup(self: *Self, message: *Message) bool {
            ...
            if (message.header.operation == .register) {
                ...
                assert(message.header.view == self.view and message.header.epoch == self.epoch);
        ...
        fn primary_pipeline_prepare(self: *Self, request: Request) void {
            ...
            if (message.header.operation == .reconfigure) {
                self.epoch_initiate_new(message.header);
            }
            ...
            while (entry.op < self.op) {
                if (self.journal.next_entry(entry)) |next| {
                    if (entry.checksum == next.parent) {
                        assert(vsr.view_order_or_eql(entry, next));
        ...
        fn primary_repair_pipeline_read(self: *Self) void {
            ...
            self.journal.read_prepare(repair_pipeline_read_callback, op, op_checksum, null, null);
        }
        ...
        fn send_prepare_ok(self: *Self, header: *const Header) void {
            ...
            assert(vsr.view_order_or_eql(header, self));
            assert(header.op <= self.op or vsr.view_order_strict(header, self));
            ...
            if (header.op > self.op) {
                assert(vsr.view_order_strict(header, self));
        ...
        fn send_start_view_change(self: *Self) void {
            ...
            if (self.standby()) return;
            log.debug("{}: send_start_view_change {} {} {} {}", .{ self.replica, self.view, self.view_durable(), self.epoch, self.epoch_durable() });
        ...
        fn send_message_to_replica_epoch(self: *Self, epoch: u32, replica: u8, message: *Message) void {
            ...
            const replica_id = self.index_to_id(epoch, replica);

            // TODO According to message.header.command, assert on the destination replica.
            switch (message.header.command) {
                .reserved => unreachable,
                .request => {
                    // FIXME: temporary allow standby to route to primary.
                    // assert(!self.standby());
            ...
            if (self.epoch_transitioning()) {
                switch (message.header.command) {
                    .ping, .pong, .prepare, .headers, .request_headers, .end_epoch => {},
                    else => return,
                }
            }
        ...
        fn view_durable_update(self: *Self) void {
            ...
            log.debug("{}: view_durable_update: epoch_durable={}..{} view_durable={}..{} log_view_durable={}..{}", .{
                self.replica,
                self.epoch_durable(),
                self.epoch,
                self.view_durable(),
        ...
        fn view_durable_update_callback(context: *SuperBlock.Context) void {
            ...
            const update_epoch = self.epoch_durable() < self.epoch;

            if (update_dvc or update_sv or update_epoch) self.view_durable_update();
        ...
        fn set_op_and_commit_max(self: *Self, op: u64, commit_max: u64, method: []const u8) void {
            ...
            if (op < self.op) {
                log.err("{}: op={} commit_max={}", .{ self.replica, op, std.math.max(commit_max, self.commit_max) });
        ...
        fn primary_log_do_view_change_quorum(
            ...
            for (dvcs_all.constSlice()) |dvc| {
                log.debug(
                    "{}: {s}: dvc: replica={} log_view={} epoch={} op={} commit_min={} checkpoint={} {}",
                    .{
                        self.replica,
                        context,
                        dvc.header.replica,
                ...
                for (dvc_headers.slice) |*header, i| {
                    log.debug(
                        "{}: {s}: dvc: header: replica={} epoch={} op={} checksum={} nack={}",
                        .{
                            self.replica,
                            context,
                            dvc.header.replica,
        ...
        fn primary_start_view_as_the_new_primary(self: *Self) void {
            ...
            if (head.operation == .reconfigure) {
                if (self.commit_max < self.op) {
                    self.epoch_initiate_new(head);
                }
            }
        }

        fn epoch_transitioning(self: *Self) bool {
            return self.epoch_old != null and self.op < self.epoch_old.?.final_prepare.op;
        }

        fn epoch_initiate_new(self: *Self, header: *const Header) void {
            assert(!self.epoch_pending);
            assert(self.epoch_old == null);
            assert(self.primary());
            assert(header.operation == .reconfigure);
            assert(self.op == header.op);
        ...
        fn epoch_abort(self: *Self) void {
            self.epoch_pending = false;
        }

        fn epoch_switch_to_new(self: *Self, message: *const Message) void {
            assert(message.header.command == .prepare or message.header.command == .end_epoch);
            if (message.header.command == .prepare) assert(message.header.operation == .reconfigure);

            assert(self.status == .normal or self.status == .view_change);
            if (self.status == .normal and self.primary()) assert(self.epoch_pending);

            assert(self.epoch_old == null);
            assert(!self.end_epoch_message_timeout.ticking);

            const members_new = message_body_as_members(message);

            const replica_new = for (members_new) |member, index| {
                if (member == self.replica_id) break @intCast(u8, index);
            } else unreachable;
            log.err("{}: epoch_switch_to_new replica_new={} op={}", .{ self.replica, replica_new, message.header.op });

            self.epoch_old = .{
                .members = self.members,
                .replica = self.replica,
                .final_prepare = message.header.*,
                .epoch = self.epoch,
            };
            self.members = members_new.*;
            self.epoch += 1;
            self.log_view = 0;
            // TODO: pick a higher view to tighten up asserts?
            self.view = 0;

            switch (message.header.command) {
                .prepare => assert(self.commit_min == self.commit_max and self.commit_min == message.header.op),
                .end_epoch => {
                    self.set_op_and_commit_max(self.op, message.header.op, "epoch_switch_to_new");
                },
                else => unreachable,
            }
            ...
            self.clock.replica = if (replica_new < self.replica_count) replica_new else 0;
            self.clock.reset();

            self.transition_to_view_change_status(self.view + 1);

            // Prevent CTRL truncating prepares from the old epoch.
            assert(self.epoch_old.?.final_prepare.op <= self.commit_max);
    ...
    fn verify(dvc_quorum: QuorumMessages) void {
        ...
        for (dvcs.constSlice()) |dvc_a, i| {
            for (dvcs.constSlice()[0..i]) |dvc_b| {
                assert(dvc_a.header.checksum != dvc_b.header.checksum);
                assert(dvc_a.header.epoch == dvc_b.header.epoch);
    ...
    fn quorum_headers(dvc_quorum: QuorumMessages, options: struct {
        ...
        const op_head = while (op <= op_head_max) : (op += 1) {
            const header_canonical = for (dvcs_canonical_.constSlice()) |dvc| {
                ...
                // Headers from past epochs are committed.
                if (op > op_head_min) assert(header.epoch == options.epoch);
            ...
            if (nacks >= options.quorum_nack_prepare) {
                if (op == op_head_min) {
                    log.err("not truncating {}", .{op});
                    // During reconfiguration, it is possible to collect a nack quorum for the final
                    // op of the old epoch, but truncating this op would be wrong. It is committed,
                    // and its durability is guaranted by the still active old epoch.
                    if (header_canonical) |op_head_min_header| {
                        assert(op_head_min_header.epoch == options.epoch - 1);
                        assert(op_head_min_header.operation == .reconfigure);
                    }
                } else {
                    log.err("truncating {}", .{op});
                    // Never nack op_head_min (aka commit_max).
                    assert(op > op_head_min);
                    // Never nack prepares from the previous epoch.
                    if (header_canonical) |op_head_min_header| {
                        assert(op_head_min_header.epoch == options.epoch);
                    }
                    break op - 1;
                }
            }
...
fn message_body_as_headers(message: *const Message) []const Header {
    ...
    for (headers) |*header| {
        ...
        assert(vsr.view_order_or_eql(header, message.header));
...
fn message_body_as_members(message: *const Message) *const [constants.nodes_max]u128 {
    switch (message.header.command) {
        .prepare => assert(message.header.operation == .reconfigure),
        .end_epoch => {},
        else => unreachable,
    }
