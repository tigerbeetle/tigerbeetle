...
// Split this into contect and SortedRun collection.
...
pub fn SortedRunsType(
    ...
    return struct {
        ...
        pub fn runs_used(self: *SortedRuns) [][]const Value {
...
pub fn TableMemoryType(comptime Table: type) type {
    ...
    return struct {
        ...
        pub fn init(
            ...
            table.shadow = try allocator.alloc(Value, Table.value_count_max);
            errdefer allocator.free(table.shadow);
        }
        ...
        pub fn runs(table: *TableMemory) [][]const Value {
            return table.sorted_runs.runs_used();
        }
        ...
        pub fn absorb(
            ...
            table_immutable.value_context.count = sort_suffix_from_offset(
                table_immutable.values[0..tables_combined_count],
                table_immutable.shadow[0..tables_combined_count],
                0,
            );
            assert(table_immutable.count() <= tables_combined_count);

            table_immutable.sorted_runs.reset();
            table_immutable.sorted_runs.add(table_immutable.values_used());

            if (constants.verify) table_immutable.sorted_runs.invariants(table_immutable.values_used());
        ...
        fn mutable_sort_suffix_from_offset(table: *TableMemory, offset: u32) []const Value {
            ...
            const target_count = sort_suffix_from_offset(table.values_used(), table.shadow[0..table.count()], offset);
