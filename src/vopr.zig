...
pub fn main() !void {
    ...
    while (tick < cli_args.ticks_max_requests) : (tick += 1) {
        ...
        if (simulator.core_missing_quorum()) {
            output.warn("no liveness, too many recovering_head replicas", .{});
            return;
...
pub const Simulator = struct {
    ...
    pub fn core_missing_prepare(simulator: *const Simulator) ?vsr.Header.Prepare {
        ...
        for (simulator.cluster.replicas) |replica| {
            if (simulator.core.isSet(replica.replica) and !replica.standby()) {
                // Lagging replicas may not make progress on commit if they always forfeit view
                // change (see `on_do_view_change` in replica.zig).
                if (replica.op_checkpoint() < cluster_op_checkpoint and
                    vsr.Checkpoint.durable(replica.op_checkpoint_next(), replica.commit_max))
                {
                    continue;
                }
