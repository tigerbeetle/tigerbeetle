...
func doTestClient(t *testing.T, client Client) {
	...
	t.Run("can create and return a transfer exists", func(t *testing.T) {
		...
		assert.Equal(t, true, outcomeExists[0].TransferWithOutcomeEventResultFlags().AccountBalancesSet)

		assert.Equal(t, outcome[0].Timestamp, outcomeExists[0].Timestamp)
		assert.Equal(t, outcome[0].Amount, outcomeExists[0].Amount)
		assert.Equal(t, outcome[0].CreditAccountDebitsPending, outcomeExists[0].CreditAccountDebitsPending)
		assert.Equal(t, outcome[0].CreditAccountDebitsPosted, outcomeExists[0].CreditAccountDebitsPosted)
		assert.Equal(t, outcome[0].CreditAccountCreditsPending, outcomeExists[0].CreditAccountCreditsPending)
		assert.Equal(t, outcome[0].CreditAccountCreditsPosted, outcomeExists[0].CreditAccountCreditsPosted)
		assert.Equal(t, outcome[0].DebitAccountDebitsPending, outcomeExists[0].DebitAccountDebitsPending)
		assert.Equal(t, outcome[0].DebitAccountDebitsPosted, outcomeExists[0].DebitAccountDebitsPosted)
		assert.Equal(t, outcome[0].DebitAccountCreditsPending, outcomeExists[0].DebitAccountCreditsPending)
		assert.Equal(t, outcome[0].DebitAccountCreditsPosted, outcomeExists[0].DebitAccountCreditsPosted)
	})
