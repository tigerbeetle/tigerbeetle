...
pub const RepairBudgetJournal = struct {
    ...
    pub fn init(gpa: std.mem.Allocator, options: struct {
        ...
        // Initialize repair latency to an arbitrarily chosen 50ms for all replicas, this gets
        // refined as we start repairing from these replicas.
        @memset(replicas_repair_latency, .ms(50));
    ...
    pub fn repair_latency(budget: *RepairBudgetJournal, replica_index: u8) stdx.Duration {
        budget.assert_invariants();
        defer budget.assert_invariants();
        return budget.replicas_repair_latency[replica_index];
    }
