...
const client_release = vsr.constants.state_machine_config.release;
...
pub fn ContextType(
    ...
    return struct {
        ...
        fn packet_enqueue(self: *Context, packet: *Packet) void {
            ...
            const batch: struct {
                ...
                inline else => |operation_comptime| blk: {
                    const event_size: u32 = comptime StateMachine.event_size_bytes(
                        client_release,
                        operation_comptime,
                    );
                    comptime assert(event_size > 0);

                    const result_size: u32 = comptime StateMachine.result_size_bytes(
                        client_release,
                        operation_comptime,
                    );
                    ...
                    const batch_event_max: u32 = StateMachine.batch_event_max(
                        client_release,
                        operation_comptime,
                        self.batch_size_limit.?,
                    );
                    if (event_count > batch_event_max) {
                        return self.notify_completion(packet, error.TooMuchData);
                    }

                    const batch_result_max: u32 = StateMachine.batch_result_max(
                        client_release,
                        operation_comptime,
                    );
                    const result_count_expected: u32 = StateMachine.batch_results_expected(
                        client_release,
                        operation_comptime,
                        slice,
                    );
                    if (result_count_expected > batch_result_max) {
                        return self.notify_completion(packet, error.TooMuchData);
                    }
        ...
        fn packet_send(self: *Context, packet_list: *Packet) void {
            ...
            const event_size: u32, const result_size: u32 = switch (operation) {
                .pulse, .get_events => unreachable,
                inline else => |operation_comptime| .{
                    @sizeOf(StateMachine.EventType(operation_comptime)),
                    @sizeOf(StateMachine.ResultType(operation_comptime)),
                },
            };
        ...
        fn client_result_callback(
            ...
            switch (operation) {
                ...
                inline else => |operation_comptime| {
                    ...
                    const result_size: u32 = @sizeOf(StateMachine.ResultType(operation_comptime));
