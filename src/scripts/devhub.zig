...
pub fn main(shell: *Shell, gpa: std.mem.Allocator, cli_args: CliArgs) !void {
    ...
    // multiversion binaries. Pick 500 as an arbitrary run number, that's significantly higher than
    // what the real release run number will be for now (~200).
    try shell.zig(
        \\build scripts -- release --build --run-number=500 --sha={sha}
        \\    --language=zig
    , .{ .sha = cli_args.sha });
