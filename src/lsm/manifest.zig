    ...
    return struct {
        ...
        pub fn equal(table: *const TreeTableInfo, other: *const TreeTableInfo) bool {
            ...
            return table.checksum == other.checksum and
                ...
                table.tree_id == other.tree_id and
        ...
        pub fn encode(table: *const TreeTableInfo) TableInfo {
            var key_min = std.mem.zeroInit(TableInfo.KeyPadded);
            var key_max = std.mem.zeroInit(TableInfo.KeyPadded);

            stdx.copy_disjoint(
                .inexact,
                u8,
                std.mem.asBytes(key_min),
                std.mem.asBytes(table.key_min),
            );

            stdx.copy_disjoint(
                .inexact,
                u8,
                std.mem.asBytes(key_max),
                std.mem.asBytes(table.key_max),
            );
...
pub fn ManifestType(comptime Table: type, comptime Storage: type) type {
    ...
    return struct {
        ...
        pub fn insert_table(
            ...
            manifest.manifest_log.insert(log_level, table.encode());
        ...
        pub fn move_table(
            ...
            manifest.manifest_log.insert(@as(u7, @intCast(level_b)), table.encode());
        ...
        pub fn remove_invisible_tables(
            ...
            while (it.next()) |table| {
                ...
                manifest.manifest_log.remove(@as(u7, @intCast(level)), table.encode());
