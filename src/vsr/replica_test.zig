...
test "Cluster: upgrade: operation=upgrade near trigger-minus-bar" {
    ...
    for ([_]struct {
        ...
        .{
            // Since there is a non-upgrade request in the last bar, the replica cannot upgrade that
            // checkpoint_1 and must pad ahead to the next checkpoint.
        ...
        t.replica(.R_).stop();
        try t.replica(.R_).open_upgrade(&[_]u16{ 2, 1 });
        ...
        t.run();
        try expectEqual(t.replica(.R_).op_checkpoint(), 0);
        try expectEqual(t.replica(.R_).release_running(), 1);

        t.replica(.R_).pass(.__, .bidirectional, .prepare_ok);
        t.run();
        t.run();
        t.run();
        try expectEqual(t.replica(.R_).release_running(), 2);
...
test "Cluster: upgrade: state-sync to new release" {
    ...
    t.replica(.R_).stop();
    try t.replica(.R0).open_upgrade(&[_]u16{ 2, 1 });
    try t.replica(.R1).open_upgrade(&[_]u16{ 2, 1 });
    try c.request(checkpoint_2_trigger, checkpoint_2_trigger);

    // R2 state-syncs from R0/R1, updating its release from v1 to v2 via CheckpointState...
    try t.replica(.R2).open();
    try expectEqual(t.replica(.R2).health(), .up);
    try expectEqual(t.replica(.R2).release_running(), 1);
    ...
    try t.replica(.R2).open_upgrade(&[_]u16{ 2, 1 });
    try expectEqual(t.replica(.R2).health(), .up);
    try expectEqual(t.replica(.R2).release_running(), 2);
    try expectEqual(t.replica(.R2).commit(), checkpoint_2);
...
test "Cluster: upgrade: R=1" {
    ...
    try expectEqual(t.replica(.R0).release_running(), 2);
...
const TestReplicas = struct {
    ...
    pub fn open(t: *const TestReplicas) !void {
        for (t.replicas.const_slice()) |r| {
            ...
            t.cluster.restart_replica(r, .{
                .releases_bundled = t.cluster.replicas[r].releases_bundled.const_slice(),
            }) catch |err| {
    ...
    pub fn release_running(t: *const TestReplicas) u16 {
        return t.get(.release_running);
    }
