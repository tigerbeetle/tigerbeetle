...
pub const Storage = struct {
    ...
    pub fn init(allocator: mem.Allocator, size: u64, options: Storage.Options) !Storage {
        ...
        try reads.ensureTotalCapacity(constants.iops_write_max);
    ...
    pub fn step(storage: *Storage) bool {
        ...
        for (order) |kind| switch (kind) {
            .read => {
                if (storage.reads.peek()) |read| {
                    if (read.ready_at.ns <= storage.tick_instant().ns) {
            ...
            .write => {
                if (storage.writes.peek()) |write| {
                    if (write.ready_at.ns <= storage.tick_instant().ns) {
