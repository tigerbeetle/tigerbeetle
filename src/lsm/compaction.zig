...
/// This is five blocks --- the index block of the output tables, and one and index & value block
/// each for the two input tables currently being merged.
pub const compaction_block_count_bar_max: u32 = 1 + (1 + 1) * 2;
...
pub fn CompactionType(
    ...
    return struct {
        ...
        level_a_index_block_carry_over: RingBufferType(*ResourcePool.Block, .{
            .array = 1,
        }) = .{ .buffer = undefined },
        ...
        level_a_value_block_carry_over: RingBufferType(*ResourcePool.Block, .{
            .array = 1,
        }) = .{ .buffer = undefined },
        ...
        level_b_index_block_carry_over: RingBufferType(*ResourcePool.Block, .{
            .array = 1,
        }) = .{ .buffer = undefined },
        ...
        level_b_value_block_carry_over: RingBufferType(*ResourcePool.Block, .{
            .array = 1,
        }) = .{ .buffer = undefined },
        ...
        pub fn idle(compaction: *const Compaction) bool {
            return compaction.pool == null and compaction.callback == null and
                // level_a
                compaction.level_a_index_block.count <=
                compaction.level_a_index_block_carry_over.buffer.len and
                compaction.level_a_value_block.count <=
                compaction.level_a_value_block_carry_over.buffer.len and
                compaction.level_a_index_block_carry_over.count == 0 and
                compaction.level_a_value_block_carry_over.count == 0 and
                // level_b
                compaction.level_b_index_block.count <=
                compaction.level_b_index_block_carry_over.buffer.len and
                compaction.level_b_value_block.count <=
                compaction.level_b_value_block_carry_over.buffer.len and
                compaction.level_b_index_block_carry_over.count == 0 and
                compaction.level_b_value_block_carry_over.count == 0 and
                // output
        ...
        fn beat_complete(compaction: *Compaction) void {
            ...
            assert(compaction.level_a_index_block_carry_over.empty());
            assert(compaction.level_a_value_block_carry_over.empty());
            assert(compaction.level_b_index_block_carry_over.empty());
            assert(compaction.level_b_value_block_carry_over.empty());
        ...
        fn compaction_dispatch_beat_table_done(compaction: *Compaction) void {
            ...
            while (compaction.level_a_value_block.head()) |block| {
                ...
                if (compaction.level_a_value_block_carry_over.count <
                    compaction.level_a_value_block_carry_over.buffer.len)
                {
                    compaction.level_a_value_block_carry_over.push_assume_capacity(block);
                } else {
                    block.stage = .free;
            ...
            while (compaction.level_a_value_block_carry_over.pop()) |block| {
                assert(block.stage == .read_value_block_done);
                compaction.level_a_value_block.push_assume_capacity(block);
            }

            assert(compaction.level_a_value_block.count <=
                compaction.level_a_value_block_carry_over.buffer.len);
            ...
            while (compaction.level_a_index_block.head()) |block| {
                ...
                if (compaction.level_a_index_block_carry_over.count <
                    compaction.level_a_index_block_carry_over.buffer.len)
                {
                    compaction.level_a_index_block_carry_over.push_assume_capacity(block);
                } else {
                    block.stage = .free;
                    compaction.pool.?.block_release(block);
                }
            ...
            while (compaction.level_a_index_block_carry_over.pop()) |block| {
                assert(block.stage == .read_index_block_done);
                compaction.level_a_index_block.push_assume_capacity(block);
            }
            assert(compaction.level_a_index_block.count <=
                compaction.level_a_index_block_carry_over.buffer.len);
            ...
            while (compaction.level_b_value_block.head()) |block| {
                ...
                if (compaction.level_b_value_block_carry_over.count <
                    compaction.level_b_value_block_carry_over.buffer.len)
                {
                    compaction.level_b_value_block_carry_over.push_assume_capacity(block);
                } else {
                    block.stage = .free;
                    compaction.pool.?.block_release(block);
                }
            ...
            while (compaction.level_b_value_block_carry_over.pop()) |block| {
                assert(block.stage == .read_value_block_done);
                compaction.level_b_value_block.push_assume_capacity(block);
            }

            assert(compaction.level_b_value_block.count <=
                compaction.level_b_value_block_carry_over.buffer.len);
            ...
            while (compaction.level_b_index_block.head()) |block| {
                ...
                if (compaction.level_b_index_block_carry_over.count <
                    compaction.level_b_index_block_carry_over.buffer.len)
                {
                    compaction.level_b_index_block_carry_over.push_assume_capacity(block);
                } else {
                    block.stage = .free;
                    compaction.pool.?.block_release(block);
                }
            ...
            while (compaction.level_b_index_block_carry_over.pop()) |block| {
                assert(block.stage == .read_index_block_done);
                compaction.level_b_index_block.push_assume_capacity(block);
            }
            assert(compaction.level_b_index_block.count <=
                compaction.level_b_index_block_carry_over.buffer.len);
        ...
        fn merge_callback(next_tick: *Grid.NextTick) void {
            ...
            compaction.counters.in += (merge_result.consumed_a + merge_result.consumed_b);
